...
# MOTD-AARCH64 Quick Setup Script
# URL: https://github.com/alphagoones/motd-aarch64
# Usage: curl -fsSL https://raw.githubusercontent.com/alphagoones/motd-aarch64/main/quick-setup.sh | bash

set -e

# Configuration
GITHUB_REPO="alphagoones/motd-aarch64"
GITHUB_API="https://api.github.com/repos/$GITHUB_REPO"
GITHUB_RAW="https://raw.githubusercontent.com/$GITHUB_REPO/main"
TEMP_DIR="/tmp/motd-aarch64-setup"
...
quick_configure() {
    ...
    echo -e "${CYAN}Choix de configuration rapide:${NC}"
    echo "1) Configuration complète (recommandée)"
    echo "2) Configuration minimale"
    echo "3) Configuration personnalisée"
    echo
    
    read -p "Votre choix (1-3) [1]: " -r choice
    choice=${choice:-1}
    ...
    case $choice in
        ...
        3)
            ...
            ;;
        *)
            print_warning "Choix invalide, utilisation de la configuration complète"
            QUICK_CONFIG="full"
            ;;
    esac
}
...
run_installation() {
    ...
    case $QUICK_CONFIG in
        "full")
            ...
            # Préparer une configuration automatique complète
            ...
            export MOTD_SERVICES="ssh,docker,nginx,apache2,fail2ban"
            ;;
        "minimal")
            ...
            export MOTD_SYSTEM_INFO="system,uptime,memory,disk"
...
show_post_install_info() {
    ...
    echo
    echo -e "${GREEN}✅ MOTD-AARCH64 installé avec succès !${NC}"
    echo
    echo -e "${CYAN}📋 Commandes utiles :${NC}"
    echo -e "  ${YELLOW}•${NC} Tester le MOTD      : ${WHITE}sudo run-parts /etc/update-motd.d/${NC}"
    echo -e "  ${YELLOW}•${NC} Reconfigurer       : ${WHITE}sudo motd-aarch64-install --configure${NC}"
    echo -e "  ${YELLOW}•${NC} Désinstaller       : ${WHITE}sudo motd-aarch64-install --uninstall${NC}"
    echo
    echo -e "${CYAN}📁 Fichiers de configuration :${NC}"
    echo -e "  ${YELLOW}•${NC} Configuration      : ${WHITE}/etc/motd-aarch64/config${NC}"
    echo -e "  ${YELLOW}•${NC} Informations       : ${WHITE}/etc/motd-aarch64/system_info${NC}"
    echo -e "  ${YELLOW}•${NC} Services           : ${WHITE}/etc/motd-aarch64/services${NC}"
    echo
    ...
    if [[ -x "/etc/update-motd.d/00-motd-aarch64" ]]; then
    ...
    echo
    echo -e "${GREEN}🎉 Profitez de votre nouveau MOTD !${NC}"
}
...
test_installation() {
    ...
    if [[ ! -f "/etc/update-motd.d/00-motd-aarch64" ]]; then
...
show_help() {
    ...
    echo "  --theme=THEME       Thème de couleurs (default|blue|green|red)"
    ...
    echo "  $0 --full           # Installation complète automatique"
...
show_version() {
    echo "MOTD-AARCH64 Quick Setup v1.0.0"
    if [[ "$LATEST_VERSION" != "unknown" ]]; then
        echo "Dernière version MOTD-AARCH64: $LATEST_VERSION"
    fi
}
