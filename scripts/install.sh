...
# MOTD-AARCH64 - Script d'installation
# https://github.com/alphagoones/motd-aarch64
...
print_banner() {
    echo -e "${CYAN}"
    echo "╔══════════════════════════════════════════════════════════════╗"
    echo "║                    MOTD-AARCH64 INSTALLER                   ║"
    echo "║              Configuration MOTD moderne pour ARM64          ║"
    echo "╚══════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
check_requirements() {
    ...
    if [[ $(uname -m) != "aarch64" ]]; then
        print_warning "Ce script est optimisé pour aarch64, mais peut fonctionner sur d'autres architectures"
...
install_packages() {
    ...
    apt update
    apt install -y figlet lolcat neofetch curl bc dialog
...
interactive_config() {
    ...
    dialog --title "Informations Système" \
           --checklist "Sélectionnez les informations à afficher:" 15 70 8 \
    ...
    SERVICES_LIST=""
    COMMON_SERVICES=("ssh" "apache2" "nginx" "docker" "fail2ban" "ufw" "postgresql" "mysql" "redis" "mongodb")
    ...
    if [[ -n "$SERVICES_LIST" ]]; then
        eval "dialog --title \"Services à surveiller\" \
               --checklist \"Sélectionnez les services à surveiller:\" 15 70 10 \
    ...
    dialog --title "Thème de couleurs" \
           --radiolist "Choisissez un thème de couleurs:" 10 60 3 \
           "default" "Thème par défaut (multicolore)" on \
           "blue" "Thème bleu" off \
           "green" "Thème vert" off \
           "red" "Thème rouge" off 2>"$CONFIG_DIR/theme.tmp"
...
generate_motd_script() {
    ...
    cat > "$MOTD_DIR/00-motd-aarch64" << 'EOF'
#!/bin/bash

# MOTD-AARCH64 - Script généré automatiquement
# Configuration: /etc/motd-aarch64/
...
get_system_info() {
    ...
    if is_enabled "temp"; then
        ...
        if [[ -f /sys/class/thermal/thermal_zone0/temp ]]; then
            ...
            CPU_TEMP=$(/opt/vc/bin/vcgencmd measure_temp | cut -d'=' -f2)
        fi
    fi
}
...
is_enabled "temp" && [[ -n "$CPU_TEMP" ]] && create_line "Température" "$CPU_TEMP"
...
EOF
    ...
    chmod +x "$MOTD_DIR/00-motd-aarch64"
...
# MOTD-AARCH64 - Services monitoring
CONFIG_DIR="/etc/motd-aarch64"
source "$CONFIG_DIR/config" 2>/dev/null || true
...
disable_old_motd() {
    ...
    for script in "$MOTD_DIR"/*; do
        if [[ -f "$script" && -x "$script" ]]; then
            ...
            if [[ "$basename_script" != "00-motd-aarch64" && "$basename_script" != "10-motd-services" ]]; then
                chmod -x "$script"
            fi
        fi
    done
...
configure_only() {
    print_banner
    print_step "Reconfiguration de MOTD-AARCH64..."
    
    if [[ ! -d "$CONFIG_DIR" ]]; then
        print_error "MOTD-AARCH64 n'est pas installé. Utilisez : sudo $0"
        exit 1
    fi
...
uninstall_motd() {
    print_banner
    print_step "Désinstallation de MOTD-AARCH64..."
    
    # Supprimer nos scripts
    rm -f "$MOTD_DIR/00-motd-aarch64"
    ...
    print_success "MOTD-AARCH64 désinstallé"
}
